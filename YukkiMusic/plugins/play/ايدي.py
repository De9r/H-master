...
async def sourc(client: Client, message: Message):
    await message.reply_photo(
        ...
        caption=f"""✧ 𝑾𝒆𝒍𝒄𝒐𝒎𝒆 𝑻𝒐 Rose ♪\n\n• ᴅᴇᴠᴇʟᴏᴘᴇʀ -› @jj8jjj8 \n• ᴄʜᴀɴɴᴇʟ Rose -› @cn_world \n\n**- للتنصيب او للاستفسار تواصل مع مطور السورس**""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                       "مطور السورس", user_id=1160471152)
                ],[
                    InlineKeyboardButton(
                       "قناة روز", url=f"https://t.me/cn_world")
...
async def aboutd5ev(client: Client, message: Message):
    usr = await client.get_chat(1160471152)
    name = usr.first_name
    bio = (await client.get_chat(1160471152)).bio
    async for photo in client.iter_profile_photos(1160471152, limit=1):
                    await message.reply_photo(photo.file_id, caption=f"""- 𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓 ♪ -› @jj8jjj8 \n\n- 𝑫𝒆𝒗𝒆𝒍𝒐𝒑𝒆𝒓'𝒔 𝑩𝒊𝒐 -› {bio}""", 
        reply_markup=InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
