...
@app.on_message(filters.command(["روزة"]) & filters.group & SUDOERS)
@language
...
async def gstats_global(client, message: Message, _):
    ...
    title = title.title()
    final = f"اكثر اغنية تم تشغيلها بروز\n\n**العنوان :** {title}\n\nتم تشغيلها** {co}0 **مرة"
...
@app.on_message(filters.command("احصائيات روز العالمية", [".", ""]) & filters.group)
@languageCB
async def gstats_global(client, message: Message, _):
    mystic = await message.reply_text(_["gstats_1"])
    stats = await get_global_tops()
    ...
    def get_stats():
        ...
        if what in ["Tracks", "Here"]:
            for items, count in list_arranged.items():
                ...
                if items == "telegram":
                    msg += f"- [مقاطع صوتية](https://t.me/v4sbot) ** تم تشغيلها {count}0 مرة**\n\n"
                else:
